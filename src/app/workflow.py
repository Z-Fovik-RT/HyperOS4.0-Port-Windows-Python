...
def resolve_work_paths(work_dir: str | Path) -> tuple[Path, Path, Path, Path]:
    ...
    if not requested.is_absolute():
        requested = Path(__file__).resolve().parents[2] / requested
...
def execute_porting(args, logger: logging.Logger) -> int:
    ...
    if not args.skip_preflight:
        ...
        report_path_arg = Path(args.preflight_report).expanduser()
        if not report_path_arg.is_absolute():
            report_path_arg = Path(__file__).resolve().parents[2] / report_path_arg
        report_path = save_preflight_report(preflight_report, report_path_arg)
