...
class Repacker:
    def __init__(self, context: Any):
        ...
        self.bin_dir: Path = Path(__file__).resolve().parents[2] / "bin"
        self.selinux_patcher: ContextPatcher = ContextPatcher()
        self.fix_timestamp: str = "1230768000"
        self.out_dir: Path = self.bin_dir.parent / "out"
        ...
        self.ota_tools_dir: Path = self.bin_dir.parent / "otatools"
    ...
    def _avb_env(self) -> Dict[str, str]:
        ...
        env["PATH"] = os.pathsep.join((ota_bin, env.get("PATH", "")))
        return env

    def _tool(self, name: str) -> Path:
        """Resolve an OTA/Android host tool for the current platform."""
        return self.shell.get_binary_path(name)
    ...
    def _get_dir_size(self, path: Path) -> int:
        """Calculate directory size without requiring the Unix ``du`` command."""
        total = 0
    ...
    def _get_free_blocks(self, img_path: Path) -> int:
        """Read free blocks when tune2fs is available; zero is a valid fallback."""
        try:
    ...
    def pack_super_image(self) -> None:
        ...
        lpmake_path: Path = self._tool("lpmake")
        ...
        # 询问用户是否继续压缩打包
        try:
            answer = input("super.img 已生成。是否继续压缩并生成刷机包？[y/N]: ").strip().lower()
        except EOFError:
            answer = ""
        if answer not in ("y", "yes"):
            self.logger.info("用户选择跳过压缩打包。super.img 保留在 %s", super_img)
            return
    ...
    def _build_avb_misc_lines_from_stock(self, partition_list: List[str]) -> List[str]:
        ...
        stock_images_dir = Path("build/stockrom/images")
        vbmeta_system_info = self._run_avbtool_info_image(
            self._tool("avbtool"), stock_images_dir / "vbmeta_system.img"
        )
        boot_info = self._run_avbtool_info_image(
            self._tool("avbtool"), stock_images_dir / "boot.img"
        )
        recovery_info = self._run_avbtool_info_image(
            self._tool("avbtool"), stock_images_dir / "recovery.img"
        )
    ...
    def _run_ota_tool(self) -> None:
        ...
        try:
            self.shell.run(
                [
                    str(self._tool("ota_from_target_files")),
                    "-v",
                    "-k",
                    str(key_path),
