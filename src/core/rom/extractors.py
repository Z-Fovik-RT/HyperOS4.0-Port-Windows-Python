...
def extract_payload(
    ...
    """
    payload_input = package.path
    # payload-dumper-go consumes payload.bin, not the enclosing OTA ZIP.
    # Extract it once into the Windows work directory when necessary.
    if package.path.is_file() and zipfile.is_zipfile(package.path):
        with zipfile.ZipFile(package.path, "r") as archive:
            if "payload.bin" in archive.namelist():
                payload_input = package.work_dir / "payload.bin"
                if not payload_input.exists():
                    package.logger.info(f"[{package.label}] Extracting payload.bin from OTA ZIP...")
                    with archive.open("payload.bin") as source, payload_input.open("wb") as target:
                        shutil.copyfileobj(source, target, length=16 * 1024 * 1024)

    cmd = ["payload-dumper", "--output", str(package.images_dir)]
