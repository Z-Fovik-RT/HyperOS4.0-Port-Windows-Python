...
def install_partition(ctx: "PortingContext", part_name: str, source_rom: RomPackage) -> None:
    ...
    try:
        _copy_partition_tree(ctx, src_dir, dest_dir)
...
def _copy_partition_tree(ctx: "PortingContext", src_dir, dest_dir) -> None:
    """Copy a partition tree with the fastest native host tool."""
    if os.name == "nt":
        # robocopy treats exit codes 0-7 as success; 8+ means real failure.
        cmd = [
            "robocopy",
            str(src_dir),
            str(dest_dir),
        ...
        result = ctx.shell.run(cmd, check=False, capture_output=True)
