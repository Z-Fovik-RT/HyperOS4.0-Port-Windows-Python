...
class PayloadDumperRunner:
    ...
    def _get_payload_dumper_path(self) -> str:
        ...
        if project_bin.exists() and not (system.startswith("win") and project_bin.read_bytes()[:4] == b"\x7fELF"):
            return str(project_bin)

        # Windows distributions may keep the executable in the bundled flash
        # tools directory or expose a .exe through PATH.
        if system.startswith("win"):
            for candidate in (
                project_root / "bin" / "windows" / "payload-dumper.exe",
                project_root / "bin" / "windows" / arch_dir / "payload-dumper.exe",
                project_root / "bin" / "flash" / "platform-tools-windows" / "payload-dumper.exe",
            ):
                if candidate.exists():
